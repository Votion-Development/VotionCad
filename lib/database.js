...
module.exports = {
    ...
    getAllBolosRaw: async function () {
        return new Promise(async (resolve, reject) => {
            const collection = db.collection("bolos");
            const filteredDocs = await collection.find({}).toArray();
            resolve(filteredDocs)
        })
    },
    ...
    setLEO: async function (id, body) {
        return new Promise(async (resolve, reject) => {
            const collection = db.collection("characters");
            await collection.updateOne({ id: id }, { $set: { leo: true, department: body.department, callsign: body.callsign, status: "10-42", rank: body.rank, supervisor: body.supervisor } });
            resolve(true)
        })
    },
    removeLEO: async function (id) {
        return new Promise(async (resolve, reject) => {
            const collection = db.collection("characters");
            await collection.updateOne({ id: id }, { $set: { leo: false, department: null, callsign: null, status: null, rank: null, supervisor: false } });
            resolve(true)
        })
    },
